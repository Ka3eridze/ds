...
public:
    ...
    // если есть доступные копии Ц выдаЄм
    // если нет Ц добавл€ем в очередь
    ...
    // возврат книги
    // если очередь пуста€ Ц увеличиваем available
    // если очередь непуста€ Ц сразу выдаЄм следующему
    void returnBook(int bookId) {
        ...
        if (!node->front) {
            // никого не ждЄт Ц просто увеличиваем available (но не больше total)
            if (node->availableCopies < node->totalCopies) {
                node->availableCopies++;
            ...
            // кто-то ждЄт Ц выдаЄм следующему
